	...
	def remove_global_msg(self):
		...
		enc_msg = re.search(
			...
			"(?:Messages to this chat and calls are now secured \
				...
				As mensagens e chamadas desta conversa estão protegidas com a \
				criptografia de ponta a ponta. Toque para mais informações.)",
			self.raw)
				
		if enc_msg: self.raw = self.raw.replace(enc_msg.group(0), "")
